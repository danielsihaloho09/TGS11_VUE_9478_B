    ...
    <v-card-text>
      <v-container>
        <v-row>
          ...
          <v-col cols="12" sm="6" md="4">
            <v-select v-model="editedItem.categories" :items="items" label="Categories"></v-select>
          </v-col>
          <v-col cols="12" sm="6" md="4">
          ...
          <v-col cols="12" sm="6" md="4">
            ...
            <v-col cols="12" sm="6" md="4">
              <v-text-field v-model="editedItem.protein" label="Protein (g)">
                </v-text-field></v-col></v-row></v-container></v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
...
export default {
  ...
  { text: "Mark", value:"multi", sortable: false },
    ...
    dialog (val) {
      val || this.close()
        ...
        methods: {
                ...
                save() {
                  if(this.cek>-1) {
                    drinksRef.child(this.editedIndex).set({name: this.editedItem.name, categories: this.editedItem.categories,
                    fat: this.editedItem.fat, carbs:this.editedItem.carbs, protein: this.editedItem.protein})
                    this.clear()
                  } else {
                    drinksRef.push({name: this.editedItem.name, categories: this.editedItem.categories, 
                    fat: this.editedItem.fat, carbs: this.editedItem.carbs, protein: this.editedItem.protein})
                    this.clear()
                  }
                  this.close()
